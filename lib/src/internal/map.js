...
class Map {
    ...
    update() {
        this.width = Math.floor(this.fromWidthToText(window.innerWidth*0.8,0)[1]);
        this.height = Math.floor(this.fromWidthToText(0,window.innerHeight)[0]);
        this.map = Array.from({ length: this.height }, () => Array(this.width).fill('.'));
    }
    draw() {
        this.update();
        this.drawer.draw();
        const window = this.map.map(row => row.join('')).join('<br>');
        return window;
    }
    addRows(rows) {
